...
private:
  CGOpenMPRuntimeColossus::ExecutionMode SavedExecMode =
      CGOpenMPRuntimeColossus::EM_Unknown;
  CGOpenMPRuntimeColossus::ExecutionMode &ExecMode;
  bool SavedRuntimeMode = false;
  bool *RuntimeMode = nullptr;

public:
  /// Constructor for Non-SPMD mode.
  ExecutionRuntimeModesRAII(CGOpenMPRuntimeColossus::ExecutionMode &ExecMode)
      : ExecMode(ExecMode) {
    SavedExecMode = ExecMode;
    ExecMode = CGOpenMPRuntimeColossus::EM_NonSPMD;
  }
  ...
  ExecutionRuntimeModesRAII(CGOpenMPRuntimeColossus::ExecutionMode &ExecMode,
    ...
    ExecMode = CGOpenMPRuntimeColossus::EM_SPMD;
...
static llvm::Value *getNVPTXWarpID(CodeGenFunction &CGF) {
  ...
  auto &RT = static_cast<CGOpenMPRuntimeColossus &>(CGF.CGM.getOpenMPRuntime());
...
CGOpenMPRuntimeColossus::ExecutionMode
CGOpenMPRuntimeColossus::getExecutionMode() const {
  return CurrentExecutionMode;
}

static CGOpenMPRuntimeColossus::DataSharingMode
getDataSharingMode(CodeGenModule &CGM) {
  return CGM.getLangOpts().OpenMPCUDAMode ? CGOpenMPRuntimeColossus::CUDA
                                          : CGOpenMPRuntimeColossus::Generic;
}
...
void CGOpenMPRuntimeColossus::emitNonSPMDKernel(const OMPExecutableDirective &D,
  ...
  public:
    NVPTXPrePostActionTy(CGOpenMPRuntimeColossus::EntryFunctionState &EST)
        : EST(EST) {}
    void Enter(CodeGenFunction &CGF) override {
      auto &RT =
          static_cast<CGOpenMPRuntimeColossus &>(CGF.CGM.getOpenMPRuntime());
...
void CGOpenMPRuntimeColossus::emitKernelInit(CodeGenFunction &CGF,
...
void CGOpenMPRuntimeColossus::emitKernelDeinit(CodeGenFunction &CGF,
                                          EntryFunctionState &EST,
                                          bool IsSPMD) {
  if (!IsSPMD)
...
void CGOpenMPRuntimeColossus::emitSPMDKernel(const OMPExecutableDirective &D,
                                          StringRef ParentName,
                                          llvm::Function *&OutlinedFn,
                                          llvm::Constant *&OutlinedFnID,
                                          bool IsOffloadEntry,
  ...
  public:
    NVPTXPrePostActionTy(CGOpenMPRuntimeColossus &RT,
                         CGOpenMPRuntimeColossus::EntryFunctionState &EST)
        : RT(RT), EST(EST) {}
    void Enter(CodeGenFunction &CGF) override {
...
void CGOpenMPRuntimeColossus::createOffloadEntry(llvm::Constant *ID,
                                              llvm::Constant *Addr,
...
unsigned CGOpenMPRuntimeColossus::getDefaultLocationReserved2Flags() const {
...
void CGOpenMPRuntimeColossus::emitProcBindClause(CodeGenFunction &CGF,
                                              ProcBindKind ProcBind,
                                              SourceLocation Loc) {
  // Do nothing in case of SPMD mode and L0 parallel.
  if (getExecutionMode() == CGOpenMPRuntimeColossus::EM_SPMD)
    return;

  CGOpenMPRuntime::emitProcBindClause(CGF, ProcBind, Loc);
}

void CGOpenMPRuntimeColossus::emitNumThreadsClause(CodeGenFunction &CGF,
                                                llvm::Value *NumThreads,
                                                SourceLocation Loc) {
  // Nothing to do.
}

void CGOpenMPRuntimeColossus::emitNumTeamsClause(CodeGenFunction &CGF,
                                              const Expr *NumTeams,
                                              const Expr *ThreadLimit,
                                              SourceLocation Loc) {}

llvm::Function *CGOpenMPRuntimeColossus::emitParallelOutlinedFunction(
...
llvm::Function *CGOpenMPRuntimeColossus::emitTeamsOutlinedFunction(
    const OMPExecutableDirective &D, const VarDecl *ThreadIDVar,
    OpenMPDirectiveKind InnermostKind, const RegionCodeGenTy &CodeGen) {
...
void CGOpenMPRuntimeColossus::emitGenericVarsProlog(CodeGenFunction &CGF,
                                                 SourceLocation Loc,
                                                 bool WithSPMDCheck) {
  if (getDataSharingMode(CGM) != CGOpenMPRuntimeColossus::Generic &&
      getExecutionMode() != CGOpenMPRuntimeColossus::EM_SPMD)
    return;
...
void CGOpenMPRuntimeColossus::emitGenericVarsEpilog(CodeGenFunction &CGF,
                                                 bool WithSPMDCheck) {
  if (getDataSharingMode(CGM) != CGOpenMPRuntimeColossus::Generic &&
      getExecutionMode() != CGOpenMPRuntimeColossus::EM_SPMD)
    return;
...
void CGOpenMPRuntimeColossus::emitTeamsCall(CodeGenFunction &CGF,
                                         const OMPExecutableDirective &D,
                                         SourceLocation Loc,
                                         llvm::Function *OutlinedFn,
...
void CGOpenMPRuntimeColossus::emitParallelCall(CodeGenFunction &CGF,
                                          SourceLocation Loc,
                                          llvm::Function *OutlinedFn,
                                          ArrayRef<llvm::Value *> CapturedVars,
                                          const Expr *IfCond,
...
void CGOpenMPRuntimeColossus::syncCTAThreads(CodeGenFunction &CGF) {
...
void CGOpenMPRuntimeColossus::emitBarrierCall(CodeGenFunction &CGF,
                                           SourceLocation Loc,
...
void CGOpenMPRuntimeColossus::emitCriticalRegion(
...
static llvm::Value *createRuntimeShuffleFunction(CodeGenFunction &CGF,
  ...
  CGBuilderTy &Bld = CGF.Builder;
  CGOpenMPRuntimeColossus &RT =
      *(static_cast<CGOpenMPRuntimeColossus *>(&CGM.getOpenMPRuntime()));
...
///
void CGOpenMPRuntimeColossus::emitReduction(
...
const VarDecl *
CGOpenMPRuntimeColossus::translateParameter(const FieldDecl *FD,
...
Address
CGOpenMPRuntimeColossus::getParameterAddress(CodeGenFunction &CGF,
...
void CGOpenMPRuntimeColossus::emitOutlinedFunctionCall(
...
llvm::Function *CGOpenMPRuntimeColossus::createParallelDataSharingWrapper(
...
void CGOpenMPRuntimeColossus::emitFunctionProlog(CodeGenFunction &CGF,
                                              const Decl *D) {
  if (getDataSharingMode(CGM) != CGOpenMPRuntimeColossus::Generic)
    return;
...
Address CGOpenMPRuntimeColossus::getAddressOfLocalVariable(CodeGenFunction &CGF,
                                                        const VarDecl *VD) {
...
void CGOpenMPRuntimeColossus::functionFinished(CodeGenFunction &CGF) {
  FunctionGlobalizedDecls.erase(CGF.CurFn);
  CGOpenMPRuntime::functionFinished(CGF);
}

void CGOpenMPRuntimeColossus::getDefaultDistScheduleAndChunk(
...
void CGOpenMPRuntimeColossus::getDefaultScheduleAndChunk(
    CodeGenFunction &CGF, const OMPLoopDirective &S,
...
void CGOpenMPRuntimeColossus::adjustTargetSpecificDataForLambdas(
...
bool CGOpenMPRuntimeColossus::hasAllocateAttributeForGlobalVar(const VarDecl *VD,
                                                            LangAS &AS) {
  if (!VD || !VD->hasAttr<OMPAllocateDeclAttr>())
    return false;
  const auto *A = VD->getAttr<OMPAllocateDeclAttr>();
...
void CGOpenMPRuntimeColossus::processRequiresDirective(
...
void CGOpenMPRuntimeColossus::clear() {
...
llvm::Value *CGOpenMPRuntimeColossus::getGPUNumThreads(CodeGenFunction &CGF) {
  CGBuilderTy &Bld = CGF.Builder;
...
llvm::Value *CGOpenMPRuntimeColossus::getGPUThreadID(CodeGenFunction &CGF) {
...
llvm::Value *CGOpenMPRuntimeColossus::getGPUWarpSize(CodeGenFunction &CGF) {
  ArrayRef<llvm::Value *> Args{};
