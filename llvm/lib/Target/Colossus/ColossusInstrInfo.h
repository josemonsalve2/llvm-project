//===-- ColossusInstrInfo.h - Colossus Instruction Information --*- C++ -*-===//
...
#ifndef LLVM_LIB_TARGET_COLOSSUS_COLOSSUSINSTRINFO_H
#define LLVM_LIB_TARGET_COLOSSUS_COLOSSUSINSTRINFO_H
...
class ColossusInstrInfo : public ColossusGenInstrInfo {
  const ColossusRegisterInfo RI;
  virtual void anchor();
...
public:
  ...
  void expandI32ToF32(MachineBasicBlock &MBB,
                      MachineBasicBlock::iterator MI,
                      unsigned Opcode) const;

  void expandF32OComparison(MachineBasicBlock &MBB,
                            MachineBasicBlock::iterator MI,
                            unsigned Opcode) const;

  void expandF32UComparison(MachineBasicBlock &MBB,
                            MachineBasicBlock::iterator MI,
                            unsigned Opcode) const;
  ...
  virtual bool canBeCSECandidate(const MachineInstr &MI) const override;

  /// Materialise constant values in a register/registers.
  void
  loadConstant32(MachineBasicBlock &MBB,
                 MachineBasicBlock::iterator MI,
                 unsigned Reg,
                 uint32_t Value,
                 bool IsRegDead = false) const;

  /// Create a machine-memory operand for a specific frame index.
  MachineMemOperand* getFrameIndexMMO(MachineBasicBlock &MBB,
                                      int FrameIndex,
