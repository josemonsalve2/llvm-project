...
std::string
ForRangeSection::render( ValueMap &valueByName ) {
        std::string result = "";
//        bool nameExistsBefore = false;
        if( valueByName.find( varName ) != valueByName.end() ) {
            std::cerr << "variable " + varName + " already exists in this context" << std::endl;
            return result;
        }
...
std::string 
ForSection::render( ValueMap &valueByName ) {
    std::string result = "";
    if( valueByName.find( varName ) != valueByName.end() ) {
        std::cerr << "variable " + varName + " already exists in this context" << std::endl;
        return result;
    ...
    if (!tupValue) {
        std::cerr <<"variable " + tupVarName + " no longer valid in context"  << std::endl;
        return result;
    }
