...
void SimUDRuntime_t::initPythonInterface(EmulatorLogLevel printLevel) {
  ...
  upstream_pyintf = new Upstream_PyIntf*[MachineConfig.NumUDs];
  sendmap = new uint32_t**[MachineConfig.NumUDs];
    ...
    for (unsigned int ln = 0; ln < MachineConfig.NumLanes; ln++) {
      ...
      sendmap[ud][ln] =
          (uint32_t *)mmap(NULL /*addr*/, 262144 * sizeof(uint32_t) /*lenght*/,
                          ...
                          fd /*file_descriptor*/, 0 /*offset*/);

      if (sendmap[ud][ln] == MAP_FAILED) {
...
void SimUDRuntime_t::send_event(event_t ev) {
  ...
  if (ev.get_NumOperands() != 0) {
    for (uint8_t i = 0; i < ev.get_NumOperands() + 1; i++)
      upstream_pyintf[ev.get_UdId()]->insert_operand(ev.get_OperandsData()[i],
                                      ev.get_LaneId());
  } else {
    upstream_pyintf[ev.get_UdId()]->insert_operand(0, ev.get_LaneId());
  }
  upstream_pyintf[ev.get_UdId()]->insert_event(ev.get_EventWord(), ev.get_NumOperands(),
                                ev.get_UdId(), ev.get_LaneId());
}


void SimUDRuntime_t::executeSingleLane(uint8_t ud_id, uint8_t lane_num) {
  UPDOWN_INFOMSG("Executing a new event in UD %d lane %d. events left = %d", ud_id, lane_num,
                  upstream_pyintf[ud_id]->getEventQ_Size(lane_num));

  emulator_stats stats;
  int exec_state = upstream_pyintf[ud_id]->execute(0, &stats, lane_num);
  ...
  // exec_state is:
  //   -1    -> Yeld terminate. If messeges to send check stat.num_sends
  ...
  if (exec_state > 0 || ((exec_state == -1) && (stats.num_sends > 0))) {
    uint32_t numsend = sendmap[ud_id][lane_num][0];
    UPDOWN_INFOMSG("UD %d Lane: %d Messages to be sent - numMessages :%d",
                    ud_id, lane_num, numsend);
    ...
    for (int i = 0; i < numsend; i++) {
      ...
      uint32_t dest_ud_id; // Updown ID for continuation
      ...
      uint32_t mode = sendmap[ud_id][lane_num][offset++];
      smode_0 = (mode & 0x1);
      smode_1 = (mode & 0x2) >> 1;
      smode_2 = (mode & 0x4) >> 2;
      ...
      uint32_t sm_cycle =
          sendmap[ud_id][lane_num][offset++]; //[8*i+2]; Not used here

      sevent = sendmap[ud_id][lane_num][offset++]; //[8*i+3];
      ...
      if (smode_0 == 0) { // DRAM Memory bound
        ...
        sdest = sendmap[ud_id][lane_num][offset++];         //[8*i+4];
        uint32_t temp = sendmap[ud_id][lane_num][offset++]; //[8*i+5];
      ...
      if (smode_0 == 1) {
        ...
        for (int j = 0; j < ssize / 4; j++) {
          sdata_32[j] = sendmap[ud_id][lane_num][offset++]; //[8*i+8+j];
...
void SimUDRuntime_t::t2ud_memcpy(void* data, uint64_t size, uint8_t ud_id,
  ...
  for (int i = 0; i < size/sizeof(word_t); i++) {
    // Address is local
    upstream_pyintf[ud_id]->insert_scratch(addr, *data_ptr);
    addr += sizeof(word_t);
    data_ptr++;
  }
}
...
void SimUDRuntime_t::ud2t_memcpy(void* data, uint64_t size, uint8_t ud_id,
  ...
  if (python_enabled)
    upstream_pyintf[ud_id]->read_scratch(addr, reinterpret_cast<uint8_t *>(base), size);
...
bool SimUDRuntime_t::test_addr(uint8_t ud_id, uint8_t lane_num, uint32_t offset,
  ...
  if (python_enabled) {
    ...
    upstream_pyintf[ud_id]->read_scratch(addr, reinterpret_cast<uint8_t *>(base), sizeof(word_t));
  }
